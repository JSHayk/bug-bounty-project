// Auth
export const INVALID_EMAIL = "Invalid email!";
export const INCORRECT_PASSWORD = "Incorrect password!";
// Requires
export const REQUIRED_TYPE = "Type must be 1 or 2!";
export const REQUIRED_PASSWORD =
  "Password must contain digits and numeric values!";
export const REQUIRE_FILE = "You must choose a file";
// Success
export const SUCCESS_REGISTER = "You've successfully registered!";
export const SUCCESS_LOGIN = "You've successfully logined!";
export const SUCCESS_ADDED = "You've successfully added!";
export const SUCCESS_DELETED = "You've successfully deleted!";
export const SUCCESS_SEND_REPORT = "You've successfully sent a report!";
export const SUCCESS_UPLOAD = "You've successfully uploaded a file!";

// Found
export const EXIST_EMAIL = "The email is already exist";
// User
export const NOT_FOUND_USER = "There is no user ";
// Common
export const EMPTY_BODY = "The request body is empty!";
export const NOT_FOUND_PRODUCT = "There is nothing with this id";
